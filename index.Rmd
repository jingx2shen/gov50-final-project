...
```
![](total_doe_rnd_re_highlight.png)
...
```
![](total_doe_rnd_with_re_presparty.png) 
...
```
![](total_doe_rnd_by_pres_and_cong.png)
...
```
![](re_doe_rnd_by_pres_and_cong.png)
...
* The coefficient on `outlays_frac_of_gdp` is about -77.56, suggesting that for every 1% increase in outlay as a fraction of U.S. GDP, the DOE budget for renewable energy R&D decreases by `$`77.56M (2020 USD), maybe pointing to other avenues of Congressional spending besides the DOE. The corresponding p-value here is 0.149, which is larger than $\alpha = 0.05$, rendering this coefficient not statistically significant.

* Similarly, the coefficient on `crude_oil_prices` is approximately -0.283, indicating that for every `$`1 increase in the price of crude oil globally, measured per cubic meter (2021 USD), the DOE budget for renewable energy R&D decreases by `$`283,000 (2020 USD). Still, the corresponding p-value of 0.490 is larger than $\alpha = 0.05$, making this coefficient not statistically significant. 
